...
class StudentRegistrationRequest(BaseModel):
    ...
    group_id: str = Field(
        ..., description="Group/Class identifier (e.g., 'CS101-2024-Spring')"
    )
    student_id: str = Field(
        ..., description="Unique student identifier within the group"
    )
    courses: List[str] = Field(default_factory=list, description="List of course IDs")
    attendance: List[dict] = Field(
        default_factory=list, description="Initial attendance records"
    )
...
def embeddings_to_dict(
    mean_embedding: np.ndarray, embeddings_stack: np.ndarray
) -> dict:
    """
...
def dict_to_embeddings(embeddings_dict: dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    augmented_embeddings = np.array(
        embeddings_dict["augmented_embeddings"], dtype=np.float32
    )
...
async def register_student(
    ...
    attendance: str = Form(
        default="[]", description="JSON string of initial attendance records"
    ),
):
    """
    ...
    try:
        ...
        try:
            ...
            attendance_list = (
                json.loads(attendance) if isinstance(attendance, str) else attendance
            )
        ...
        if not isinstance(attendance_list, list):
            raise HTTPException(
                status_code=400, detail="Attendance must be a JSON array"
            )
        ...
        mean_embedding, embeddings_augmentations = await image_validator.final_process(
            image
        )
        ...
        if not result:
            raise HTTPException(
                status_code=500, detail="Failed to store student in database"
            )
...
async def get_student_embeddings(group_id: str, student_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve student: {str(e)}"
        )


@app.put("/api/v1/groups/{group_id}/students/{student_id}/attendance")
async def update_attendance(group_id: str, student_id: str, attendance_record: dict):
    """
    ...
    """
    try:
        result = db_service.update_student_attendance(
            group_id, student_id, attendance_record
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update attendance: {str(e)}"
        )
...
async def get_group_students(group_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve students: {str(e)}"
        )
...
async def get_all_students():
    ...
    try:
        ...
        return {"success": True, "total_students": len(students), "students": students}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve all students: {str(e)}"
        )


@app.put("/api/v1/groups/{group_id}/students/{student_id}/courses")
async def update_student_courses(group_id: str, student_id: str, courses: List[str]):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update courses: {str(e)}"
        )
...
async def delete_student(group_id: str, student_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete student: {str(e)}"
        )
...
async def store_generic_data(payload: GenericDataModel):
    ...
    try:
        if not payload.collection_name or not payload.data:
            raise HTTPException(
                status_code=400, detail="collection_name and data fields are required"
            )
        ...
        if not result.inserted_id:
            raise HTTPException(
                status_code=500, detail="Failed to store data in database"
            )
    ...
    try:
        ...
        for collection_name in collections:
            ...
            collection_stats.append(
                {"name": collection_name, "document_count": doc_count}
            )
